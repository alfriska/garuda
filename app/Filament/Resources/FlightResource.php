        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('segments')
                    ...
                    ->formatStateUsing(function (Flight $record) {
                       $firstSegment = $record->segments->first();
                       $lastSegment = $record->segments->last();
                       $route = $firstSegment->airport->iata_code . ' _ ' . $lastSegment->airport->iata_code;
                       $duration = (new \DateTime($lastSegment->time))->format('d F Y H:i') . '_' . (new \DateTime($firstSegment->time))->format('d F Y H:i');
